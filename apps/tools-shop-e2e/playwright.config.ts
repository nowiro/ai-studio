 ...
 */
const baseURL = process.env['BASE_URL'] ?? 'http://localhost:4209';
...
export default defineConfig({
  ...
  reporter: [['list'], ['html', { outputFolder: '../../dist/.playwright/apps/tools-shop-e2e/report', open: 'never' }]],
  use: {
    baseURL,
  ...
  webServer: process.env['CI']
    ? undefined
    : {
        command: 'pnpm exec nx serve tools-shop --port 4209',
        url: baseURL,
