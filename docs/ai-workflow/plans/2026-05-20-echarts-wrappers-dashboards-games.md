...
status: done
closedAt: 2026-05-20
closeReason: 'All 13 tasks delivered. T011 closed via an option-builders refactor — wrapper internals extracted to pure functions in `libs/charts/src/option-builders.ts`, covered by 20 smoke specs (line/bar/pie/gauge/heatmap shape, axis swap, stacking, legend positioning, gauge & heatmap formatters). T013 closed via fresh `apps/dashboard-e2e` (homepage 4 panels visible, `/charts/showcase` 5 canvas wrappers) plus `apps/pong-game-e2e/src/settings.spec.ts` and `apps/tetris-game-e2e/src/{settings,leaderboard}.spec.ts`; both game configs gained a Mobile Chrome (Pixel 7) project alongside Desktop Chrome. E2E run command: `pnpm nx e2e dashboard-e2e|pong-game-e2e|tetris-game-e2e`. Core deliverables — libs/charts foundation, 5 wrappers, dashboard 4-panel migration, pong + tetris UI rebuild, showcase route, ADR-0016 + governance lint rule + architecture doc — all green and in production.'
owner: orchestrator
agents:
  - architect
  - frontend-developer
...
| T013 | E2E smoke: dashboard charts visible, games settings + leaderboard (chromium + mobile chrome) | test-engineer      | T008,T009,T010 | apps/dashboard-e2e + new specs in pong/tetris-game-e2e + Pixel 7 project | specs in place, lint green; user runs `pnpm nx e2e ...` locally |               | T008,T009,T010 |
