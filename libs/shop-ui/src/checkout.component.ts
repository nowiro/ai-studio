...
import { type DeliveryMethod, ShopCartService } from '@ai-studio/shop-core';
...
const DEFAULT_DELIVERY_OPTIONS: readonly { readonly value: DeliveryMethod; readonly label: string }[] = [
  { value: 'courier', label: 'Kurier (49 zł)' },
  { value: 'pickup-point', label: 'Paczkomat (19 zł)' },
  { value: 'in-store', label: 'Odbiór w sklepie (0 zł)' },
];

/**
 * Generic 4-step Reactive-Forms checkout reused by every shop. The cart
 * itself lives in `ShopCartService`; this component owns the form
 * groups, validation rules and the mock-order-placed flow.
 */
@Component({
  selector: 'ais-shop-checkout',
    ...
    >
      ...
      @if (cart.views().length === 0) {
        <div class="gap-3 py-16 flex flex-col items-center text-center text-on-surface-variant">
          <p class="m-0">Twój koszyk jest pusty. Wróć do katalogu, aby dodać produkty.</p>
          <a
          ...
          <mat-step label="Podsumowanie">
            ...
            >
              ...
              <ul class="m-0 p-0 gap-2 flex list-none flex-col">
                @for (view of cart.views(); track view.product.id) {
                  <li
                  ...
                  >
                    ...
                    <div class="text-sm flex-1">
                      <div class="font-semibold">{{ view.product.brand }} {{ view.product.name }}</div>
                      <div class="text-on-surface-variant">
                        {{ view.line.quantity }} × {{ format(view.product.priceCents) }}
                      </div>
                    </div>
...
export class CheckoutComponent {
  ...
  protected readonly deliveryOptions = DEFAULT_DELIVERY_OPTIONS;
  protected readonly catalogueRoute = '/';

  /** Order-number prefix per shop (e.g. `TS` for tire-shop, `BS` for bookstore). */
  protected readonly orderPrefix = signal<string>('ORD');
  ...
  protected readonly deliveryMethodLabel = computed(() => {
    ...
    return DEFAULT_DELIVERY_OPTIONS.find((option) => option.value === method)?.label ?? '';
  });
  ...
  protected placeOrder(): void {
    ...
    const orderNumber = `${this.orderPrefix()}-${Date.now().toString(36).toUpperCase()}`;
